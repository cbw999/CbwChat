/**
 * 이 코드는 @modelcontextprotocol/sdk를 기반으로 동작하는 서버 인스턴스를 생성하며:
 * MCP 툴을 테스트하거나 데모할 수 있는 다목적 MCP 서버입니다.
 * 다양한 유형의 툴(Tool), 프롬프트(Prompt), 리소스(Resource) 등을 선언하고 처리합니다.
 * LLM 샘플링, 리소스 구독/알림, 툴 실행, 프롬프트 조회 등을 포함한 종합 기능 테스트를 제공합니다
 * Model Context Protocol (MCP) 기반의 샘플 서버를 정의하고, 여러 기능(툴, 프롬프트, 리소스 등)을 
 * JSON-RPC로 처리할 수 있게 구성된 멀티기능 MCP 테스트 서버입니다. 
 * MCP 클라이언트가 연결하면 다양한 기능을 호출할 수 있습니다.
*/
