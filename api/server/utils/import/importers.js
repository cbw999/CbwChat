...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('cbwchat-data-provider');
 ...
 * 이 코드는 다양한 챗봇 시스템(예: ChatGPT, ChatbotUI, LibreChat)으로부터 대화 데이터를 JSON으로 임포트하여 시스템 내로 변환/저장하는
 *  importer 로직 모음입니다.
 * 주요 목적은: 외부에서 받은 대화 JSON 파일을 분석하고, 우리 시스템에 맞는 포맷으로 변환하여 DB에 저장하는 것
 *
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing CbwChat conversation');
    return importCbwChatConvo;
  }
...
/**
 * Imports a CbwChat conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importCbwChatConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
