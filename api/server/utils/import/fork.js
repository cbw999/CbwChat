...
const { EModelEndpoint, Constants, ForkOptions } = require('cbwchat-data-provider');
 ...
 * 이 코드는 **대화(conversation)**와 그에 속한 **메시지 트리 구조(message hierarchy)**를 다양한 방식으로 복제(fork)하거나,
 * 복제하면서 일부만 잘라내거나, 전체를 복제하는 유틸리티입니다.
 * 트리 기반 대화 모델링 (예: 챗봇, AI 채팅) 시스템에서 복사/포크 기능을 만들기 위해 설계된 고급 모듈입니다.
