...
/**
 * 이 코드는 메모리 누수 방지 및 클라이언트 리소스 정리를 위한 유틸리티 모음입니다.
 * 주요 대상은 대화형 AI 클라이언트 객체이며, 이 객체가 GC(Garbage Collection) 대상이 되었을 때
 * 또는 명시적으로 정리해야 할 때 안전하게 모든 내부 상태와 참조를 제거하도록 돕습니다.
 * 주요 목적
 * 사용이 끝난 클라이언트 객체의 메모리 해제 및 리소스 정리
 * 클라이언트 객체의 GC 감시 및 자동 정리(FinalizationRegistry 사용)
 * 요청 처리 중 공유되는 임시 데이터(requestDataMap) 관리
 * 요청 컨텍스트 정보에서 필요한 필드만 추출 (processReqData)
 */
/**
